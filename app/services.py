...
class Service:
    ...
    def get_users_similarity(self, user_name):
        users_fav = {u[0]: set(u[1])
                     for u in self._model.get_users_favorites_list()}
    ...
    def get_movie_relations(self, movie_name, count, exponent=0.55):
        ...
        for user, favorites in self._model.get_users_favorites_list():
            if movie_name in favorites:
                for other in favorites:
                    ...
                    dd[other] += self._model.get_favorited_count(
                        other)**-exponent
        sort = sorted(dict(dd).items(), key=lambda item: item[1], reverse=True)
        score_dict = OrderedDict(sort[:count])
        if movie_name in score_dict.keys():
            score_dict.pop(movie_name)
        return (score_dict)
    
    def get_movie_relations_n_data(self, movie_name, count, exponent=0.55):
        movie_relations = self.get_movie_relations(
    ...
    def search_movie_name(self, names: str):
...
if __name__ == "__main__":
    ...
    for other_movie, score in list(movie_similarity.items())[:10]:
        print(f"\t{other_movie}: {score}")
